...
void MassiveCache::nextState(Tick tick)
{
    switch (state) {
    case StateIdle:
        break;
...
int64_t MassiveCache::stateLength(State state)
{
    switch (state) {
    case StateIdle:
        return 1;
...
void MassiveCache::nextTick(Tick tick)
{
...
void MassiveCache::requestCache(Request request)
{
...
bool MassiveCache::isAvailable()
{
    return (state == StateIdle or state == StateReturning);
}

std::string MassiveCache::stateToString(State _state)
{
    switch (_state) {
    case StateIdle:
        return "StateIdle";
...
void MassiveCache::changeState(State _state, Tick tick)
{
    if (state != _state) {
        char text[1000];
