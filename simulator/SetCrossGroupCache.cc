...
void SetCrossGroupCache::nextState(Tick tick)
{
    switch (state) {
    case StateIdle:
        break;
...
int64_t SetCrossGroupCache::stateLength(State state)
{
    switch (state) {
    case StateIdle:
        return 1;
...
void SetCrossGroupCache::nextTick(Tick tick)
{
...
void SetCrossGroupCache::requestCache(Request request)
{
...
bool SetCrossGroupCache::isAvailable()
{
    return (state == StateIdle or state == StateReturning);
}

std::string SetCrossGroupCache::stateToString(State _state)
{
    switch (_state) {
    case StateIdle:
        return "StateIdle";
...
void SetCrossGroupCache::changeState(State _state, Tick tick)
{
    if (state != _state) {
        char text[1000];
...
std::string SetCrossGroupCache::getStateName()
{
